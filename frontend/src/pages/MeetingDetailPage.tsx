...
import {
  ...
  SUMMARY_LOADING_MESSAGES,
} from "@/pages/meeting-detail/helpers";
...
export default function MeetingDetailPage() {
  ...
  const [summaryLoadingMessageIndex, setSummaryLoadingMessageIndex] =
    useState(0);
  ...
  const [editedSegments, setEditedSegments] = useState<Map<number, string>>(
    new Map(),
  );
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      setSummaryLoadingMessageIndex(
        (prev) => (prev + 1) % SUMMARY_LOADING_MESSAGES.length,
      );
    }, 1400);
    ...
    const fetchMeetingData = async () => {
      ...
      try {
        ...
        if (meetingResult.success && meetingResult.data) {
          ...
          throw new Error(
            meetingResult.error || "Không thể tải thông tin cuộc họp",
          );
        }
      } catch (err) {
  ...
  const handleStartTranscription = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Đang xử lý transcript",
        description: "Hệ thống đang chuyển đổi âm thanh thành văn bản...",
      });
      ...
      pollingIntervalRef.current = setInterval(async () => {
        try {
          ...
          if (!meetingResult.success || !meetingResult.data) {
            ...
            if (
              meetingResult.error?.includes("401") ||
              meetingResult.error?.toLowerCase().includes("unauthorized")
            ) {
          ...
          if (updatedMeeting.status === "DONE") {
            ...
            toast({
              title: "Transcript hoàn tất",
              description: "Bản ghi âm đã sẵn sàng.",
            });
            ...
            setTranscribing(false);
            toast({
              title: "Transcript thất bại",
              description: "Quá trình xử lý gặp lỗi.",
              variant: "destructive",
            });
          }
        } catch {
      ...
      toast({
        title: "Lỗi",
        description:
          err instanceof Error ? err.message : "Không thể bắt đầu transcript",
        variant: "destructive",
      });
    }
  }, [id, navigate, toast]);
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        toast({
          ...
          description: "Đã cập nhật tóm tắt cuộc họp.",
        });
      } else {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Không thể lấy tóm tắt. Vui lòng thử lại.";
  ...
  const handleAddDecision = useCallback(() => {
    setSummaryDraft((prev) =>
      prev
        ? { ...prev, decisions: [...prev.decisions, ""] }
        : { ...createEmptySummary(), decisions: [""] },
    );
  }, []);
  ...
  const handleTaskChange = useCallback(
    (index: number, field: "task" | "owner" | "deadline", value: string) => {
      setSummaryDraft((prev) => {
        if (!prev) return prev;
  ...
  const handleDeadlineChange = useCallback(
    (index: number, field: "date" | "item", value: string) => {
      setSummaryDraft((prev) => {
        if (!prev) return prev;
    ...
    setSummaryDraft((prev) => {
      ...
      return {
        ...
        deadlines: prev.deadlines.filter(
          (_, deadlineIndex) => deadlineIndex !== index,
        ),
      };
    });
  }, []);
  ...
  const handleSaveTranscript = useCallback(async () => {
    ...
    try {
      ...
      const editsArray = Array.from(editedSegments.entries()).map(
        ([index, text]) => ({
          index,
          text,
        }),
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <span className="font-medium">Tiêu đề:</span> {meeting.title}
              </div>
              <div>
              ...
              <div>
                <span className="font-medium">Tên file gốc:</span>{" "}
                  ...
                  <div className="flex w-full items-center justify-between gap-4 pr-4 text-left">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-muted-foreground">
                        Nhấn nút bên dưới để tóm tắt cuộc họp. Nhấp đúp vào nội
                        dung sau khi tải để chỉnh sửa nhanh.
                      </p>
                    </div>
                    {hasSummary && (
                      <div className="hidden flex-wrap items-center gap-2 lg:flex">
                        <Badge variant="secondary">
                          {summary?.decisions.length || 0} quyết định
                        </Badge>
                        <Badge variant="secondary">
                          {summary?.tasks.length || 0} đầu việc
                        </Badge>
                        <Badge variant="secondary">
                          {summary?.deadlines.length || 0} mốc
                        </Badge>
                      </div>
                    )}
                  </div>
                  ...
                  {!hasSummary && !loadingSummary ? (
                    <div className="rounded-2xl border border-dashed border-primary/20 bg-gradient-to-br from-primary/5 to-background px-6 py-10 text-center">
                      ...
                      <h3 className="text-lg font-semibold">
                        Lấy tóm tắt cấu trúc cho cuộc họp
                      </h3>
                      <p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                        Hệ thống sẽ hiển thị tổng quan, quyết định quan trọng,
                        đầu việc cần làm và các mốc thời gian để bạn rà soát
                        nhanh trước khi export.
                      </p>
                      <Button
                    ...
                    <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-primary/10 bg-gradient-to-br from-primary/5 via-background to-background px-6 py-12 text-center">
                      ...
                      <div className="space-y-1">
                        <p className="text-base font-medium">
                          Đang xử lý tóm tắt cuộc họp
                        </p>
                        <p className="text-sm text-muted-foreground">
                          {SUMMARY_LOADING_MESSAGES[summaryLoadingMessageIndex]}
                        </p>
                      </div>
                    </div>
                  ) : summaryError ? (
                    ...
                    <div className="space-y-5">
                      <div className="flex flex-col gap-3 rounded-2xl border border-primary/10 bg-gradient-to-r from-primary/5 via-background to-background p-4 md:flex-row md:items-center md:justify-between">
                        <div>
                          <p className="text-sm font-medium">
                            Dữ liệu tóm tắt đã sẵn sàng
                          </p>
                          <p className="text-sm text-muted-foreground">
                            Bạn có thể chỉnh sửa trực tiếp trên giao diện trước
                            khi xuất DOCX.
                          </p>
                        </div>
                      ...
                      {editingSummary && summaryDraft ? (
                        <div className="space-y-5">
                          <div className="rounded-2xl border bg-card p-4 shadow-sm">
                            ...
                            <Textarea
                              ...
                              onChange={(event) =>
                                handleSummaryTextChange(event.target.value)
                              }
                            ...
                            <div className="rounded-2xl border bg-card p-4 shadow-sm">
                              ...
                              <div className="space-y-3">
                                {summaryDraft.decisions.length === 0 && (
                                  <p className="text-sm text-muted-foreground">
                                    Chưa có quyết định nào. Bạn có thể thêm mới
                                    bên trên.
                                  </p>
                                )}
                                {summaryDraft.decisions.map(
                                  (decision, index) => (
                                    <div
                                      key={`decision-${index}`}
                        ...
                        <div
                          className="space-y-5"
                          onDoubleClick={handleStartSummaryEdit}
                        >
                          <div className="grid gap-5 xl:grid-cols-3">
                          ...
                          <div className="grid gap-5 xl:grid-cols-3">
                              ...
                              {summary.decisions.length > 0 ? (
                                <div className="space-y-3">
                                  {summary.decisions.map((decision, index) => (
                                    <div
                                      key={`decision-read-${index}`}
                                      className="rounded-xl border border-emerald-100 bg-emerald-50/60 p-3 text-sm leading-6 text-foreground/90"
                                    >
                            ...
                            <div className="rounded-2xl border bg-card p-5 shadow-sm xl:col-span-2">
                              ...
                              {summary.tasks.length > 0 ? (
                                  ...
                                  {summary.tasks.map((task, index) => (
                                    ...
                                    >
                                      <p className="text-sm font-medium leading-6 text-foreground">
                                        {task.task || "Chưa có mô tả công việc"}
                                      </p>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  Chưa có đầu việc nào được trích xuất.
                                </p>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="rounded-2xl border bg-card p-5 shadow-sm">
                            ...
                            {summary.deadlines.length > 0 ? (
                              ...
                              <p className="text-sm text-muted-foreground">
                                Chưa có mốc thời gian nào trong phần summary.
                              </p>
                            )}
                          </div>
                        </div>
                ...
                {transcribing ? (
                  <>
                    ...
                    <p className="text-sm mt-1">
                      Hệ thống đang chuyển đổi âm thanh thành văn bản, vui lòng
                      chờ.
                    </p>
                  </>
                ) : (
                  <>
                    ...
                    <p className="text-sm mt-1">
                      Nhấn nút{" "}
                      <span className="font-semibold">Bắt đầu Transcript</span>{" "}
                      ở góc trên để xử lý bản ghi âm.
                    </p>
                  </>
                )}
              </div>
              ...
              <ScrollArea className="h-[600px] pr-4">
                <div className="space-y-2">
                  {segments.map((segment, index) => {
                    ...
                    const segmentRef =
                      index === activeSegmentIndex ? activeSegmentRef : null;
            ...
            <div className="space-y-4 py-4">
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setExportIncludeSummary(checked === true)
                  }
                  disabled={!hasSummary}
                />
                <Label
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setExportIncludeTranscript(checked === true)
                  }
              ...
              <div className="text-sm text-muted-foreground">
                ...
                <span className="font-mono">
                  {meeting.title || "meeting"}-{meeting.id}.docx
                </span>
              </div>
            </div>
        ...
        <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Bạn có chắc chắn muốn xóa cuộc họp "{meeting.title}"? Hành động
